  ...
  return (
    <section className="py-20">
      <div className="lg:px-44 md:px-10 lg:mt-20 dark:bg-gray-900">
        <motion.h2 className="text-5xl mb-20 font-semibold text-gray-700 dark:text-gray-200"
        ...
        >
          {sectors.map((item, index) => (
            <div key={index} className="text-center">
              {/* Image */}
              <Image src={item.image} width={80} height={80} alt={item.title} className="mx-auto mb-6" />
              ...
              <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                {item.tags.map((tag, i) => (
                  <Link href={"#"} key={i} className="text-sm bg-gray-100 dark:bg-gray-700 dark:text-gray-200 px-3 py-1 rounded">
                    {tag}
                  </Link>
                ))}
              </div>
