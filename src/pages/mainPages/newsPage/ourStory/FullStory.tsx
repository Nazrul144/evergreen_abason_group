...
const FullStory = () => {
  return (
    <section className="lg:px-44 md:px-10 px-2 lg:mt-20 dark:bg-gray-900 mb-20">
      {/* Header */}
      <motion.div
      ...
      >
        <p className="text-sm tracking-wide font-semibold text-gray-500 dark:text-gray-400 flex items-center justify-center gap-2 mt-4 lg:mt-0">
          Evergreen Abason Group
          <span className="h-px w-10 bg-green-500"></span>
        </p>

        <h1 className="text-2xl lg:text-4xl md:text-6xl font-bold mt-4 text-transparent bg-clip-text bg-linear-to-r from-emerald-400 via-sky-400 to-blue-600">
      ...
      <div className="relative mt-20 lg:mt-28">
        {/* Vertical line */}
        <div className="hidden lg:block absolute left-1/2 top-0 -translate-x-1/2 w-1 h-full bg-linear-to-b from-emerald-500 via-blue-500 to-sky-400 opacity-70" />
          ...
          {sections.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                className={`flex flex-col lg:flex-row items-start lg:gap-10 relative ${
                  isLeft ? "lg:flex-row-reverse" : "lg:flex-row"
                }`}
              >
                {/* Connecting line for cards */}
                <div
                  className={`hidden lg:block absolute top-14 ${
                    isLeft ? "right-1/2" : "left-1/2"
                  } w-20 h-1 bg-gradient-to-r from-emerald-400 to-blue-500 ${
                ...
                {/* Card */}
                <div className="w-full lg:w-1/2 bg-white dark:bg-gray-800 lg:p-8 lg:rounded-2xl shadow-lg lg:hover:shadow-emerald-500/20 lg:hover:scale-[1.02] transition-all duration-300 border border-gray-100 dark:border-gray-700 relative z-10">
                  <div className="flex items-center gap-1 lg:gap-4">
                    <div className="lg:w-14 lg:h-14 flex items-center justify-center rounded-full bg-linear-to-br from-emerald-500 to-blue-600 shadow-lg shadow-emerald-500/40">
                      {item.icon}
                    </div>
                    <h2 className=" text-sm mt-2 lg:mt-0 lg:text-2xl md:text-3xl font-semibold text-gray-900 dark:text-white">
                      {item.title}
                    </h2>
                  </div>
      ...
      >
        <Link
          href="/news"
          className="px-8 py-3 rounded-full text-white text-lg font-semibold bg-gradient-to-r from-emerald-500 to-blue-600 hover:opacity-90 hover:scale-105 transition-all duration-300 shadow-lg shadow-emerald-600/30"
        >
          Back to News
        </Link>
      </motion.div>
